...
class GNNEncoder(nn.Module):
    ...
    def forward(self, x, edge_index):
        x = self.create_input_feat(x).to(self.device)
        edge_index = to_sparse_tensor(edge_index, x.size(0)).to(self.device)
    ...
    def train_step(self, data, optimizer, alpha=0.002,
        ...
        for perm in DataLoader(
            ...
            if self.degree_decoder is not None and alpha:
                deg = degree(masked_edges[1].flatten(), data.num_nodes).float().to(z.device)
